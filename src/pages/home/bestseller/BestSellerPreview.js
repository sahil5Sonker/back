// src/pages/BestSellerPreview.jsx
...
const BestSellerPreview = () => {
  const { bestSellerPreview, setBestSellerPreview } = useContext(AppContext);
  const [loading, setLoading] = useState(true);

  const fetchPreview = async () => {
    try {
      setLoading(true);
      const res = await api.get("/api/product/bestseller?limit=10"); // ✅ API with limit param
      setBestSellerPreview(res.data.products || []);
    } catch (err) {
      console.error("Error fetching best seller preview:", err.message);
    } finally {
      setLoading(false);
    }
  };
    ...
    >
      <Box sx={{ position: "relative" }}>
        ...
        <CardMedia
          component="img"
          height="180"
          image={`${api.defaults.baseURL}/${product.image}`} // ✅ Using API baseURL
          alt={product.title}
        />
      </Box>
